...
import { getDashboardRoute, normalizeAuthError, type AppRole } from "@/lib/auth-utils";
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    setMessage({ type: "success", text: "กำลังสร้างบัญชีของคุณ..." });
    ...
    setMessage({ type: "success", text: "เราได้ส่งอีเมลยืนยันไปยังกล่องจดหมายของคุณแล้ว" });
    setPassword("");
    setConfirmPassword("");
  };
    ...
    <AuthShell
      ...
      subtitle={
        <>
          ...
          <Link className="font-extrabold underline underline-offset-4" href={loginHref}>
            เข้าสู่ระบบ
          </Link>
        </>
      }
      ...
      <form className="grid gap-5 sm:grid-cols-2" onSubmit={handleSubmit}>
        ...
        <label className="grid gap-2 text-left text-sm font-bold uppercase tracking-[0.08em]">
          ...
          <input
            ...
            value={password}
          />
          <span className="text-xs font-medium normal-case tracking-normal text-[#6b645d]">ใช้รหัสผ่านอย่างน้อย 8 ตัวอักษรเพื่อความปลอดภัย</span>
        </label>
        <label className="grid gap-2 text-left text-sm font-bold uppercase tracking-[0.08em]">
